...
| | 方案 A：本地部署 | 方案 B：远程部署 |
|---|---|---|
| **适合谁** | 个人开发者，只在一台电脑用 | 团队协作，或多设备切换 |
| **需要什么** | Mac/Linux/Windows + Docker | 一台云服务器（VPS） |
| **数据存哪** | 你的电脑本地 | 云服务器上 |
| **难度** | ⭐⭐ 简单 | ⭐⭐⭐ 中等 |

> 💡 **不确定选哪个？** 先用方案 A，随时可以迁移到方案 B。
...
> Claude Desktop 目前不支持直接连接远程 MCP 服务器（SSE 模式）。建议使用本地 stdio 模式或等待官方更新。
...
JetBrains IDE 仅支持远程 SSE 模式（方案 B），在 AI Assistant MCP 配置中添加：
...
**A**: 不会。Qdrant 和 Ollama 都使用了 Docker Volume 做持久化存储，重启/升级不会丢失数据。
...
1. **Docker 没启动** — 确保 Docker Desktop 已打开，Qdrant 和 Ollama 容器正在运行
